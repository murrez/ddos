...
func main() {
	...
	go func() {
		fmt.Println("-- https://murrez.blogspot.com --\n")
		ss := make(chan uint8, 8)
		var (
			err, sent int32
		)
		fmt.Println("Levels         /\tSend Up /\tOutput")
		for {
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d - %-6d /\t%7d /\t%6d", cur, maxproc, sent, err)
				fmt.Println("\r-- STOPED, Ohhhhhh your mom fucked server :* --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
	...
	<-ctlc
	fmt.Println("\r\n-- STOPED, Ohhhhhh your mom fucked server :* --        \n")
}
